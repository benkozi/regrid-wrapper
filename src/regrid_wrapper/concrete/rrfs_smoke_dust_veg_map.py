...
class RrfsSmokeDustVegetationMap(AbstractRegridOperation):
    ...
    def run(self) -> None:
        ...
        x_bounds = dst_grid_def.get_bounds(
            dst_grid, esmpy.StaggerLoc.CENTER, dst_grid_def.axis_order.x
        )
        y_bounds = dst_grid_def.get_bounds(
            dst_grid, esmpy.StaggerLoc.CENTER, dst_grid_def.axis_order.y
        )
        if dst_grid_def.axis_order.x == 0:
            first = x_bounds
            second = y_bounds
        else:
            first = y_bounds
            second = x_bounds
        emiss_factor[first.lower : first.upper, second.lower : second.upper] = (
            dst_field.data
        )
